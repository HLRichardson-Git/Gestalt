...
unsigned int RSAPublicKey::getPublicModulusBitLength() const {
    return static_cast<unsigned int>(n.bitLength());
}
...
bool RSAKeyPair::validatePrivateKey(RSAPrivateKey privateKeyCandidate) {
    ...
    if (!privateKeyCandidate.q.isZero()) {
        if (!isPrime(privateKeyCandidate.q)) {
            throw std::invalid_argument("'q' is not prime.");
        }
    }

    // If both p and q are prime, they are coprime by definition (distinct primes always have gcd == 1).

    unsigned int specifiedStrengthValue = static_cast<unsigned int>(specifiedStrength);
    unsigned int dBitLength = static_cast<unsigned int>(privateKeyCandidate.d.bitLength());
...
bool RSAKeyPair::validatePublicKey(RSAPublicKey publicKeyCandidate) {
    unsigned int nBitLength = static_cast<unsigned int>(publicKeyCandidate.n.bitLength());
    ...
    if (publicKeyCandidate.e < 65536 || publicKeyCandidate.e > twoPow256) {
...
void RSAKeyPair::generateKeyPair(RSAKeyGenOptions options) {
    unsigned int halfBits = static_cast<unsigned int>(options.securityStrength) / 2;

    BigInt p = generateLargePrime(halfBits, options.primeMethod);
    BigInt q = generateLargePrime(halfBits, options.primeMethod);

    BigInt n     = p * q;
    BigInt phi_n = (p - 1) * (q - 1);

    privateKey.d = publicKey.e.modInverse(phi_n);
    privateKey.p = p;
    privateKey.q = q;
...
unsigned int RSAKeyPair::getModulusBitLength() const {
    return static_cast<unsigned int>(publicKey.n.bitLength());
}

unsigned int RSAKeyPair::getPrivateExponentBitLength() const {
    return static_cast<unsigned int>(privateKey.d.bitLength());
}
