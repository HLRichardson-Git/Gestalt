...
TEST(DES_ECB, encrypt) {
    SecureBytes ciphertext = encryptDESECB(plaintext, key);
    SecureBytes expected = SecureBytes::fromHex("fca4a0b8cb579ea8cc830fcf3a5ee2dc");
    EXPECT_EQ(ciphertext, expected);
}

TEST(DES_ECB, decrypt) {
    SecureBytes ciphertext = SecureBytes::fromHex("fca4a0b8cb579ea8cc830fcf3a5ee2dc");
    SecureBytes decryptedPlaintext = decryptDESECB(ciphertext, key);
    EXPECT_EQ(decryptedPlaintext, plaintext);
}

TEST(DES_ECB, multiBlock) {
    SecureBytes ciphertext = encryptDESECB(multiBlockPT, key);
