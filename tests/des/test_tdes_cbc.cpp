...
TEST(TDES_CBC, encrypt) {
    SecureBytes ciphertext = encrypt3DESCBC(plaintext, nonce, key, key2, key3);
    SecureBytes expected = SecureBytes::fromHex("ee6edc51099b7783bf57f381d620957c");
    EXPECT_EQ(ciphertext, expected);
}

TEST(TDES_CBC, decrypt) {
    SecureBytes ciphertext = SecureBytes::fromHex("ee6edc51099b7783bf57f381d620957c");
    SecureBytes decryptedPlaintext = decrypt3DESCBC(ciphertext, nonce, key, key2, key3);
    EXPECT_EQ(decryptedPlaintext, plaintext);
}

TEST(TDES_CBC, multiBlock) {
    SecureBytes ciphertext = encrypt3DESCBC(multiBlockPT, nonce, key, key2, key3);
