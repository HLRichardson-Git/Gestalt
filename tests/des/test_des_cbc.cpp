...
TEST(DES_CBC, encrypt) {
    SecureBytes ciphertext = encryptDESCBC(plaintext, nonce, key);
    SecureBytes expected = SecureBytes::fromHex("95a32bce039b97b209e35f005da93c0c");
    EXPECT_EQ(ciphertext, expected);
}

TEST(DES_CBC, decrypt) {
    SecureBytes ciphertext = SecureBytes::fromHex("95a32bce039b97b209e35f005da93c0c");
    SecureBytes decryptedPlaintext = decryptDESCBC(ciphertext, nonce, key);
    EXPECT_EQ(decryptedPlaintext, plaintext);
}

TEST(DES_CBC, multiBlock) {
    SecureBytes ciphertext = encryptDESCBC(multiBlockPT, nonce, key);
