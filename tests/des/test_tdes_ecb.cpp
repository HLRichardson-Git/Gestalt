...
TEST(TDES_ECB, encrypt) {
    SecureBytes ciphertext = encrypt3DESECB(plaintext, key, key2, key3);
    SecureBytes expected = SecureBytes::fromHex("8b3a49695d593b3633f5d3a48c4de370");
    EXPECT_EQ(ciphertext, expected);
}

TEST(TDES_ECB, decrypt) {
    SecureBytes ciphertext = SecureBytes::fromHex("8b3a49695d593b3633f5d3a48c4de370");
    SecureBytes decryptedPlaintext = decrypt3DESECB(ciphertext, key, key2, key3);
    EXPECT_EQ(decryptedPlaintext, plaintext);
}

TEST(TDES_ECB, multiBlock) {
    SecureBytes ciphertext = encrypt3DESECB(multiBlockPT, key, key2, key3);
