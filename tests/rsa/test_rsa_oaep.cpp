...
TEST(RSA_OAEP, applyOAEP) {
    SecureBytes output = applyOAEP_Padding(
        SecureBytes::fromAscii("Hello, Gestalt!"),
        OAEPParams(HashAlgorithm::SHA1, MGF1, SecureBytes{}, SecureBytes::fromHex("aafd12f659cae63489b479e5076ddec2f06cb58f")),
        80);
    ...
    EXPECT_TRUE(output.toHex() == expectedOutput);
}
...
TEST(RSA_OAEP, removeOAEP) {
    std::string input =
        "00db9178b344652251240b0ee59f8b33a8d882089fdcd87d5c68f1eea8f55267c31b2e8bb4251f84d7e0b2c04626f5aff93edcfb25c9c2"
        "b3ff8ae10e839a2ddb4d949b2398180494e6d2b2424ac6a6bb";
    SecureBytes output = removeOAEP_Padding(SecureBytes::fromHex(input), OAEPParams(HashAlgorithm::SHA1, MGF1), 80);
    EXPECT_EQ(output, SecureBytes::fromAscii("Hello, Gestalt!"));
}
    ...
    EXPECT_THROW({
        try {
            std::string input =
                ...
                "0fb25c9c2b3ff8ae10e839a2ddb4cdcfe4ff47728b4a1b7c1362baad29ab48d2869d5024121435811591be392f982fb3e87d09";
            SecureBytes output = applyOAEP_Padding(SecureBytes::fromHex(input), OAEPParams(HashAlgorithm::SHA1, MGF1), 80);
...
TEST(RSA_OAEP, messageDecodeHasWrongLabel) {
    EXPECT_THROW({
        try {
            ...
            SecureBytes output = removeOAEP_Padding(SecureBytes::fromHex(input), OAEPParams(HashAlgorithm::SHA1, MGF1, SecureBytes::fromAscii("badLabel")), 80);
        } catch (const std::invalid_argument& e) {
...
TEST_P(RSA_OAEP_Test, encrypt) {
    ...
    SecureBytes computedCiphertext = rsa.encrypt(SecureBytes::fromHex(test.pt), test.publicKey, test.parameters);

    EXPECT_TRUE(computedCiphertext == SecureBytes::fromHex(test.ct));
}
...
TEST_P(RSA_OAEP_Test, decrypt) {
    ...
    RSA rsa(test.keySecurityStrength, test.privateKey, test.publicKey);
    SecureBytes computedPlaintext = rsa.decrypt(SecureBytes::fromHex(test.ct), test.parameters);

    EXPECT_TRUE(computedPlaintext == SecureBytes::fromHex(test.pt));
}
