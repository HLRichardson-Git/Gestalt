...
TEST(RSA_OAEP, applyOAEP) {
    SecureBytes output = applyOAEP_Padding(
        ...
        OAEPParams(HashAlgorithm::SHA1, MGF1, "", SecureBytes::fromHex("aafd12f659cae63489b479e5076ddec2f06cb58f")),
        80);
...
TEST(RSA_OAEP, messageDecodeHasWrongLabel) {
    EXPECT_THROW({
        try {
            ...
            SecureBytes output = removeOAEP_Padding(SecureBytes::fromHex(input), OAEPParams(HashAlgorithm::SHA1, MGF1, "badLabel"), 80);
        } catch (const std::invalid_argument& e) {
